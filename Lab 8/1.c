/*
    ...
    Објаснување на примерот: Во секоја линија лево од стрелката се печати секој број од интервалот од [10,20], а десно
    од стрелката се печати наредниот број поголем од бројот лево којшто е делив со бројот k.
...
int nextDivisibleByK(int number,int k){
    int novSledenDeliv;
    novSledenDeliv = number + 1;
    if(divisibleByK(novSledenDeliv, k)){
        return novSledenDeliv;
    }
    nextDivisibleByK(novSledenDeliv, k);
}
